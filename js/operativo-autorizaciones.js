    ...
    const render = (items) => {
        ...
        list.innerHTML = items.map((item) => `
            <article class="op-approval-card">
                ...
                <div class="op-card-actions">${item.decision === 'Pendiente' ? `<button class="op-success-button" data-approve="${item.id}"><i class="fa-solid fa-check"></i> Aprobar</button><button class="op-danger-button" data-reject="${item.id}"><i class="fa-solid fa-xmark"></i> Rechazar</button>` : ''}</div>
            </article>`).join('');
